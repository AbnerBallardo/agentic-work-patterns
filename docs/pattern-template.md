# Pattern Template

Copy this structure into `patterns/<pattern-name>/README.md`.

```markdown
# Pattern Name

## Status and Evidence Basis

- **Status:** Prototype / Field-informed / Field-tested
- **Evidence basis:**
- **Not established:**

## Agent Definition

- **Definition(s) used:**
- **Why the definition applies:**
- **Interaction model:**
- **Knowledge and tool access:**
- **External actions:**
- **Autonomy boundary:**
- **Stopping conditions:**
- **Human handoff and decision authority:**

## Organizational Problem

Describe the recurring problem and its consequence.

## Intended Users

Name the primary user and the accountable decision-maker.

## When to Use It

-

## When Not to Use It

-

## Human Responsibility

State what the agent may support and what a human must validate, own, or decide.

## Required Inputs

-

## Expected Output

-

## Agent Workflow

1.

## Failure Modes

| Failure mode | Required response |
|---|---|
| | |

## Privacy and Security Considerations

-

## Evaluation Criteria

-

## Implementation Options

Link only to implementations that have been built or tested.

## Observed Value and Limitations

Separate firsthand observation, measured results, hypotheses, and unknowns.
```

Each pattern should include vendor-neutral agent instructions, at least one synthetic example, and an evaluation rubric or test case.
